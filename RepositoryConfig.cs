...
namespace IntarRepo
{
    public class RepositoryConfig
    {
        ...
        public List<RepositoryConfigEntry> RepositoryEntries { get; private set; } = new List<RepositoryConfigEntry>();
        public bool JestBazaDanych { get; set; }

        private static readonly Regex komentarzRegex = new Regex(@"^//.*$", RegexOptions.Compiled | RegexOptions.IgnoreCase);
        private static readonly Regex headerPlusBazaDanychRegex = new Regex(@"^\s*DATABASE\s*;\s*OWNER\s*;\s*NAME\s*;\s*TYPE\s*;\s*DESCRIPTION\s*;?\s*$", RegexOptions.Compiled | RegexOptions.IgnoreCase);
        private static readonly Regex headerBezBazaDanychRegex = new Regex(@"^\s*OWNER\s*;\s*NAME\s*;\s*TYPE\s*;\s*DESCRIPTION\s*;?\s*$", RegexOptions.Compiled | RegexOptions.IgnoreCase);
        private static readonly Regex bazaDanychRegex = new Regex(@"^(BPSC|TEST|KPP)$", RegexOptions.Compiled | RegexOptions.IgnoreCase);
        private static readonly Regex ownerRegex = new Regex(@"^(BAZA_BPSC|EROZRYS|NAVIGATOR|B2B|B2C|INTEGRACJA|PUBLIC|TOPSOLID)$", RegexOptions.Compiled | RegexOptions.IgnoreCase);
        ...
        public RepositoryConfig(string filePath)
        {
            _filePath = filePath;
            ...
            try
            {
                ...
                foreach (var line in lines)
                {
                    ...
                    if (!headerChecked)
                    {
                        ...
                        if (headerPlusBazaDanychRegex.IsMatch(trimmedLine))
                        {
                            JestBazaDanych = true;

                        }
                        else if (headerBezBazaDanychRegex.IsMatch(trimmedLine))
                        {
                            JestBazaDanych = false;
                        }
                        else
                        {
                    ...
                    // Walidacja na podstawie typu pliku
                    if (JestBazaDanych)
                    {
                        ValidatePlusBazaDanych(parts, line);
                    }
                    else
                    {
                        ValidateBezBazy(parts, line);
                    }
                }
        ...
        private void ValidatePlusBazaDanych(string[] parts, string line)
        {
            var database = parts[0].Trim();
            var owner = parts[1].Trim();
            var name = parts[2].Trim();
            var type = parts[3].Trim();
            var description = parts.Length == 5 ? parts[4].Trim() : "";

            if (!bazaDanychRegex.IsMatch(database))
            {
                ValidationErrors.Add($"Invalid database: {database} in line: {line}");
            }
            ...
            RepositoryEntries.Add(new RepositoryConfigEntry(database, owner, name, type,description));
        }

        private void ValidateBezBazy(string[] parts, string line)
        {
            ...
            RepositoryEntries.Add(new RepositoryConfigEntry("KPP", owner, name, type, description));
    ...
    public class RepositoryConfigEntry
    {
        ...
        public RepositoryConfigEntry(string database, string owner, string name, string type, string description)
        {
            Database = database;
            Owner = owner;
            Name = name;
